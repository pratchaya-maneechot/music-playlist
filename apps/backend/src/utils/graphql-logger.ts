...
export function ApolloLoggerPlugin<T extends Record<string, any>>(
  ...
  const plugin: ApolloServerPlugin<T> = {
    async requestDidStart(
      initialContext: GraphQLRequestContext<T>
    ): Promise<GraphQLRequestListener<T>> {
      ...
      return {
          ...
          return {
            willResolveField({ info, args }) {
              ...
              return (error) => {
                ...
                const logLevel =
                  duration > logSlowResolversThreshold ? 'warn' : 'debug';
                _logger[logLevel](
                  {
        ...
        async willSendResponse(context) {
          ...
          const executionDuration = timings.executionStart
